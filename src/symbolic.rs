...
use super::{Bool, Model, ModelEq, ModelValue, Solver};
...
impl<T> Symbolic<T> {
    ...
    pub fn new(solver: &mut Solver, mut xs: Vec<T>) -> Symbolic<T> {
        match xs.len() {
            0 => panic!("Symbolic value cannot be initialized from empty list."),
            1 => Symbolic(vec![(true.into(), xs.remove(0))]),
            2 => {
