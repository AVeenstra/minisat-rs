...
pub trait ModelOrd {
    ...
    fn assert_less_tuple_or<B: ModelOrd>(
        ...
        match B::new_less_lit(solver, inclusive, p, q) {
            Bool::Const(c) => Self::assert_less_or(solver, prefix, c, x, y),
            lit => {
...
impl ModelOrd for Bool {
    fn assert_less_tuple_or<B: ModelOrd>(
        ...
        if x == y {
            ...
            let less_lit = B::new_less_lit(solver, inclusive, p, q);
            solver.add_clause(prefix.iter().cloned().chain(once(*y)).chain(once(less_lit)));
            solver.add_clause(
                prefix
                    .iter()
                    .cloned()
